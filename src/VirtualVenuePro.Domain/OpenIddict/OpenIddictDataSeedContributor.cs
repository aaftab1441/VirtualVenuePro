...
namespace VirtualVenuePro.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("VirtualVenuePro") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "VirtualVenuePro", DisplayName = "VirtualVenuePro API", Resources = { "VirtualVenuePro" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["VirtualVenuePro_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["VirtualVenuePro_Web:RootUrl"]!.EnsureEndsWith('/');

            /* VirtualVenuePro_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["VirtualVenuePro_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["VirtualVenuePro_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["VirtualVenuePro_Swagger:RootUrl"]?.TrimEnd('/');
